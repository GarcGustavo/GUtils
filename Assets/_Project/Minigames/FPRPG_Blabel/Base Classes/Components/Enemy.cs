...
namespace Base_Classes
{
	public class Enemy : GridUnit
	{
		...
		private void Awake()
		{
			//_dialogueTrigger = GetComponentInChildren<DialogueSystemTrigger>();
		}
		...
		protected virtual void Start()
		{
			...
			_events.unitDamage.AddListener(Damage);
			//_manager.enemyTurn.AddListener(EnemyAction);
			_events.enemyAttack.AddListener(Attack);
		...
		public override void InitializeUnit(GridCell cell)
		{
			...
			//if (!_dialogueTrigger) return;
			//_dialogueTrigger.conversationConversant = transform;
			//GetComponentInChildren<Usable>().overrideName = _unitName;
			//_dialogueTrigger.conversationActor = _manager.GetPlayer().transform;
		}
		...
		private void Attack(Vector3Int unitCell)
		{
			...
			_events.playerDamage.Invoke(_data.attack);
